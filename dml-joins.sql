...
--1. Write a query to return all the rows from user_new table and role tables where there is matching user_new.role_id value in both the usernew and role tables. Display role.name and user_new.name in ascending order.
    SELECT USER_NEW.*, ROLE.* FROM USER_NEW INNER JOIN ROLE ON USER_NEW.ROLE_ID = ROLE.ID ORDER BY ROLE.NAME, USER_NEW.NAME ASC ;
...
--2. Write a query to return all rows from the user_new table and only those rows from the role table where the joined fields are equal. Display role.name and user_new in ascending order.
    SELECT USER_NEW.* FROM USER_NEW LEFT OUTER JOIN ROLE ON USER_NEW.ID = ROLE.ID  ORDER BY ROLE.NAME ASC;
...
--3. Write a query to return all names from the user_new table where there is matching user_new.role_id value in both the user_new and role tables.
    SELECT USER_NEW.NAME FROM USER_NEW, ROLE WHERE USER_NEW.ROLE_ID = ROLE_ID;  
...
--  - Display in the order user_name.name followed by experience.company_name
 ...
 experience on profile.id = experience.profile_id where role.name='Alumini' order by  user_new.name  asc;
...
select  user_new.name, user_new.emailid, user_new.phonenumber, profile.address from user_new left join role on user_new.role_id= role.id 
 left join profile on user_new.profile_id = profile.id left join degree on degree.id = profile.degree_id  where role.name='Alumni'  and  experience.currentValue=1 order by  user_new.name  asc;
...
select  user_new.name, user_new.emailid, user_new.phonenumber, profile.address from user_new left join role on user_new.role_id= role.id 
 left join profile on user_new.profile_id = profile.id left join degree on degree.id = profile.degree_id left join experience on degree.id = experience.id where role.name='Alumni'  
...
select  user_new.name, user_new.emailid, user_new.phonenumber, profile.address from user_new left join role on user_new.role_id= role.id 
 left join profile on user_new.profile_id = profile.id left join degree on degree.id = profile.degree_id left join experience on degree.id = experience.id where role.name='Alumni'  
 and  experience.currentValue=1 and user_new.profile_id is not null order by  user_new.name  asc;
...
select user_new.name, higher_degree.university_name from user_new left join role on user_new.role_id= role.id left join profile on user_new.profile_id = profile.id left join  higher_degree 
...
select user_new.name, profile.batch, degree.name, department.name from user_new  left join role on user_new.role_id= role.id 
 left join profile on user_new.profile_id = profile.id left join degree on degree.id = profile.degree_id left join department on department.id =degree.department_id 
 where role.name='Alumni' and profile.gender='female' order by  user_new.name  asc;
...
select degree.name, department.name from degree left join department on degree.department_id = department.id order by  degree.name
...
select user_new.name, querytable.content from user_new left join querytable on querytable.user_id = user_new.id order by  user_new.name asc;
...
select user_new.name, querytable.content from user_new inner join querytable on querytable.user_id = user_new.id where querytable.parent_id IS NOT NULL order by  user_new.name asc;
...
--  - Display in the order of user_new.name;
 ...
 select user_new.name, post.content from user_new left join post on post.user_id = user_new.id order by  user_new.name asc ;
...
select user_new.name, post.content from user_new left join post on post.user_id = user_new.id inner join post_type on post_type.id = post.type_id where post_type.name like '%Technology%' order by user_new.name,post.dateof; 
...
--27. Write a query to return all user_new.name, department.name from user_new where there are matching ids in te tables 
--  - role
--  - profile
--  - degree
--  - department
...
--  - Display event in descending order.
 ...
 select user_new.name, events.name from user_new  left join role on user_new.role_id= role.id INNER JOIN EVENTS ON EVENTS.organiser_id  =  user_new .ID INNER JOIN ROLE ON  user_new.ROLE_ID = ROLE.ID
